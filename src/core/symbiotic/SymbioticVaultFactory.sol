...
import {NativeSymVault} from "../../vault/symbiotic/NativeSymVault.sol";
...
contract SymbioticVaultFactory is Initializable, OwnableUpgradeable {
    ...
    function createVault(
        ...
        // Deploy BurnerRouter
        address burnerRouter = _deployBurnerRouter(burnerRouterParams, nativeSymVault, symPod);
        ...
        // Deploy Vault
        (vault, delegator, slasher) = _deployVault(configuratorParams, vaultParams, delegatorParams, slasherParams, burnerRouter, nativeSymVault, symPod);

        // Deploy DefaultStakerRewards
        defaultStakerRewards = _deployDefaultStakerRewards(stakerRewardsParams, vault, nativeSymVault);

        // Initialize NativeSymVault and whitelist the symPod as a depositor
        NativeSymVault(nativeSymVault).initialize(nativeSymVault, vault, symPod);

        return (vault, delegator, slasher, defaultStakerRewards, nativeSymVault, symPod);
    }
    ...
    function _deployDefaultStakerRewards(
        ...
        return IDefaultStakerRewardsFactory(DEFAULT_STAKER_REWARDS_FACTORY).create(
            IDefaultStakerRewards.InitParams({
                ...
                defaultAdminRoleHolder: nativeSymVault,
                adminFeeClaimRoleHolder: nativeSymVault,
                adminFeeSetRoleHolder: nativeSymVault
            })
        );
    }
    ...
    function _initializeVaultInitParams(
        ...
        return abi.encode(IVault.InitParams({
            ...
            defaultAdminRoleHolder: nativeSymVault,
            depositWhitelistSetRoleHolder: nativeSymVault,
            depositorWhitelistRoleHolder: nativeSymVault,
            isDepositLimitSetRoleHolder: nativeSymVault,
            depositLimitSetRoleHolder: nativeSymVault
        }));
    }
    ...
    function _initializeDelegatorInitParams(
        ...
        if (configuratorParams.delegatorIndex == 0) {
            ...
            initParams.networkLimitSetRoleHolders[0] = nativeSymVault;
            initParams.operatorNetworkSharesSetRoleHolders[0] = nativeSymVault;
            ...
            initParams.networkLimitSetRoleHolders[0] = nativeSymVault;
            initParams.operatorNetworkLimitSetRoleHolders[0] = nativeSymVault;
            return abi.encode(initParams);
        }
    }

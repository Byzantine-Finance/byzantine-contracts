...
import "./EIP_4788_Oracle_Mock.t.sol";
...
contract BeaconChainMock is Test {
    ...
    *******************************************************************************/
    ...
    function exitValidator(uint40 validatorIndex) public returns (uint64 exitedBalanceGwei) {
        ...
        vm.deal(destination, address(destination).balance + uint(uint(exitedBalanceGwei) * GWEI_TO_WEI));
    ...
    function _withdrawExcess() internal {
        ...
        for (uint i = 0; i < validators.length; i++) {
            ...
            vm.deal(destination, address(destination).balance + excessBalanceWei);
    ...
    function _advanceEpoch() public {
        ...
        vm.warp(_nextEpochStartTimestamp(curEpoch));
